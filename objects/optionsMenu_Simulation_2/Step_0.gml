...
if (button[lastSelected] == "Slider:Initial Development") { //If currently on the slider, execute the slider's function. The slider's visual will automatically adjust.
	...
	if (slider_move != 0) { //If the slider was moved
		playSound(1);
		if (slider_move == 1) and (global.initialDevelopmentAmount < 105) {
			global.initialDevelopmentAmount += 10;
		} else if (slider_move == -1) and (global.initialDevelopmentAmount > 5) {
			global.initialDevelopmentAmount -= 10;
		}
	}
