...
if (is_undefined(parent) == false) and (parent != pointer_null) { //Make sure the parent exists and is not null
...
while (parentExists == false) and (ds_list_size(species.creatures) > 0) { //If the parent is null or does not exist, choose a different parent until it does exist.	
	...
	if (is_undefined(parent) == false) and (parent != pointer_null) and (instance_exists(parent) == true) {
		parentExists = instance_exists(parent);
	}
...
//Create an egg
if (ds_list_size(species.creatures) > 0) {
